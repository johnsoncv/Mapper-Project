...
public:
   ...
   */
  void update_status(std::string const &message);
  
  // Initalize the autocomplete for the two search bars
  void initializeAutoComplete();
  
  /* Resets all the cards current in the DetailBox BoxWidget */
  void resetCards();
  
  /* FOR INTERNAL USE IN APPLICATION ONLY 
   * Creates a standard card with standard styling, customize styling in custom.css
   * @param title, title of the card
   * @param subtitle, subtitle of the card
   * @param detailHeader, optional, header for details list, pass empty string to leave empty
   * @param details, string vector of additional details
   */
  GtkWidget* createCard(std::string const &title, std::string const &subtitle, std::string const &detailHeader, std::vector<std::string> const &details);
  
  /* 
   * Creates a standard card for bus prediction
   * @param estimatedTime, predicted time of bus
   */
  void createPredictionCard(std::string const &estimatedTime, std::string const &routeName, std::string const &routeNumber);
  
  /* 
   * Creates a standard card for intersection
   * @param estimatedTime, predicted time of bus
   */
  void createIntersectionCard(std::string const &name, std::string const &coords, std::vector<unsigned> nearbyPOIs);
  
  /* 
   * Creates a standard card for weather
   * @param estimatedTime, predicted time of bus
   */
  void createWeatherCard(std::string const &name, std::string const &weather, std::string const &currentTemp, std::string const &visibility);
  
  /* 
   * Creates a standard card for POI
   * @param estimatedTime, predicted time of bus
   */
  void createPOICard(std::string const &name, std::string const &coords, std::string const &type);
  
    /* 
   * Creates a standard card for Path
   * @param distance between points, travel time, turn type and segment length details
   */
  void createPathCard(std::string const &distance, std::string const &time, std::vector<std::string> const &details);
  
    /* 
   * Creates a standard card for Help Menu
   */  
  void createHelpCard();
  
    /* 
   * Creates a standard card for displaying an error was received
   */  
  void createErrorCard(std::string message);

  /**
   * Gets input from SearchBox widget
   */
  void get_input();
